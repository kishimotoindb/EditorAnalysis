...
public class TextView extends View implements ViewTreeObserver.OnPreDrawListener {
    ...
    /*
     * autoLink当中有五个属性值：分别是phone、email、map、web、all和none，前四个分别是自动识别电话号码、
     * 邮箱、地址和网址，而第五个是识别前四个的全部，none是不识别；在不设置none而设置其他值的情况下，当你的
     * TextView当中有phone/map/web/email的值的时候，并且linksClickable="true"的时候，点击TextView控件
     * 会自动打开
     */
     ...
     * 1.Editor并行处理所有事件，并且一般不返回处理结果。
     * 2.View基本的点击和长按事件，由父类处理
     *
     * ---------------------------------------------------------------------------------------------
     * onTouchEvent()方法的流程：
     * Editor.onTouchEvent()
     * View.onTouchEvent()
     * MoveMent.onTouchEvent()
     * Up事件时:linkClick、键盘弹起、Editor.onTouchUpEvent()
     *
     *
     * ---------------------------------------------------------------------------------------------
     * 特别注意一点：
     * 在长按事件或双击事件发生之前，Editor和TextView的onTouchEvent是并行执行的，所以要注意异步操作的问题，
     * 同时，performLongClick也是并行进行的。
     *
     *
     * 额外的疑问：
     * 1.显示的所有光标应该都是popUpwindow。但是选中文字的背景色是谁负责绘制的，如果是TextView，那么在绘制
     * 背景色时，一定会触发TextView的draw方法。
     *
     */
    @Override
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if ((mMovement != null || onCheckIsTextEditor()) && isEnabled()
            ...
            // important!!!
            // movement的onTouchEvent()与下面对UP事件的处理是并行的
            if (mMovement != null) {
                // 这里应该是处理的EditText移动文字时，光标跟着动
                handled |= mMovement.onTouchEvent(this, (Spannable) mText, event);
            }

            // 处理TextView中的网页链接、电话号码的点击，与Editor无关
            // isTextSelectable()和isTextEditable()（下面的if里）两个方法，isTextSelectable是TextView和
            // EditText都可能用到的，但是isTextEditable()这个方法，根据方法实现来判断，应该是只适用于EditorText，
            // 因为有一句代码：text isInstantsOf Editable，TextView
            //
            // 这里负责link等的点击处理（与Editor无关）
